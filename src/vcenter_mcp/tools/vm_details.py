...
@mcp.tool()
def get_vm_floppy(vm: str, floppy: str, vcenter_name: str | None = None) -> str:
    """Get a VM floppy device."""
    return json_response(vcenter_get(f"/api/vcenter/vm/{path_id(vm)}/hardware/floppy/{path_id(floppy)}", **resolve_vcenter(vcenter_name)))


@mcp.tool()
def list_vm_scsi_adapters(vm: str, vcenter_name: str | None = None) -> str:
    """List VM SCSI adapters."""
    return json_response(vcenter_get(f"/api/vcenter/vm/{path_id(vm)}/hardware/scsi", **resolve_vcenter(vcenter_name)))


@mcp.tool()
def get_vm_scsi_adapter(vm: str, adapter: str, vcenter_name: str | None = None) -> str:
    """Get a VM SCSI adapter."""
    return json_response(vcenter_get(f"/api/vcenter/vm/{path_id(vm)}/hardware/scsi/{path_id(adapter)}", **resolve_vcenter(vcenter_name)))


@mcp.tool()
def list_vm_sata_adapters(vm: str, vcenter_name: str | None = None) -> str:
    """List VM SATA adapters."""
    return json_response(vcenter_get(f"/api/vcenter/vm/{path_id(vm)}/hardware/sata", **resolve_vcenter(vcenter_name)))


@mcp.tool()
def get_vm_sata_adapter(vm: str, adapter: str, vcenter_name: str | None = None) -> str:
    """Get a VM SATA adapter."""
    return json_response(vcenter_get(f"/api/vcenter/vm/{path_id(vm)}/hardware/sata/{path_id(adapter)}", **resolve_vcenter(vcenter_name)))


@mcp.tool()
def list_vm_nvme_adapters(vm: str, vcenter_name: str | None = None) -> str:
    """List VM NVMe adapters."""
    return json_response(vcenter_get(f"/api/vcenter/vm/{path_id(vm)}/hardware/nvme", **resolve_vcenter(vcenter_name)))


@mcp.tool()
def get_vm_nvme_adapter(vm: str, adapter: str, vcenter_name: str | None = None) -> str:
    """Get a VM NVMe adapter."""
    return json_response(vcenter_get(f"/api/vcenter/vm/{path_id(vm)}/hardware/nvme/{path_id(adapter)}", **resolve_vcenter(vcenter_name)))
